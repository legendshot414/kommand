...
import io.github.legendshot414.kommand.AbstractKommandSuggestion
import io.github.legendshot414.kommand.ref.getValue
import io.github.legendshot414.kommand.ref.weak
...
class NMSKommandSuggestion(
    ...
    override fun suggest(value: Int, tooltip: (() -> ComponentLike)?) {
        ...
        else handle.suggest(value, PaperAdventure.asVanilla(tooltip().asComponent()))
    }

    override fun suggest(text: String, tooltip: (() -> ComponentLike)?) {
        if (tooltip == null) handle.suggest(text)
        else handle.suggest(text, PaperAdventure.asVanilla(tooltip().asComponent()))
    }
    ...
    override fun suggest(candidates: Iterable<String>, tooltip: ((String) -> ComponentLike)?) {
        ...
        candidates.forEach { candidate ->
            ...
            if (SharedSuggestionProvider.matchesSubStr(input, lowerCandidate)) {
                ...
                else handle.suggest(candidate, PaperAdventure.asVanilla(tooltip(candidate).asComponent()))
            }
        }
    }
    ...
    override fun <T> suggest(
        ...
        candidates.forEach {
            ...
            if (SharedSuggestionProvider.matchesSubStr(input, lowerCandidate)) {
                if (tooltip == null) handle.suggest(candidate)
                else handle.suggest(candidate, PaperAdventure.asVanilla(tooltip(it).asComponent()))
            }
        }
    }
    ...
    override fun <T> suggest(
        ...
        candidates.forEach { (key, value) ->
            ...
            if (SharedSuggestionProvider.matchesSubStr(input, lowerCandidate)) {
                ...
                else handle.suggest(key, PaperAdventure.asVanilla(tooltip(value).asComponent()))
            }
        }
    }
}